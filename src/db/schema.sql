...
alter table acces_externes enable row level security;-- Schéma Supabase (Postgres) pour la plateforme "Lieux hybrides et territoires".
-- À exécuter dans l'éditeur SQL de votre projet Supabase.
-- L'authentification (lien magique) est gérée par Supabase Auth (table auth.users) ;
-- on référence simplement auth.users(id) depuis nos tables.
...
-- ---- Objets Changethegame (voir migration_011_objets_ctg.sql) ----
